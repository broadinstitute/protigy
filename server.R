...
<br><p><font size=\"4\">Please note that the expression values in your data matrix should be <b>log-transformed</b>, especially in case of ratio data!</p><br><p>Now go on upload a file and enjoy your analysis.</p></font>',

'<hr><p><font size=\"5\" color=\"red\">What\'s new:</font></p>',
'<font size=\"4\">
<b>v0.4.1</b>
<ul>
<li>Added option to skip testing, i.e. for PCA analysis.</li>
</ul>
<br>
<b>v0.4</b>
<ul>
...
,sep='') )
        ...
        output$help.test <- renderText({
            ...
            HTML( paste('<font size=\"4\"><p><h3>Data normalization</h3>You can apply different normalization methods to the data prior to testing. The methods are applied for each column separately, except for \'Quantile\'-normalization which takes the entire matrix into account.</p>
<p>
<ul>
...
<br></font></p>' ) )
           ...
           ## } ## end if not mod F
            ...
            pca.tab <- tabPanel('PCA',
                                     fluidPage(
                                         ...
                                         fluidRow(column(6, align='center', plotlyOutput("pcaxy.plotly", width=500, height=500)),
                                                  column(6, align='center', plotlyOutput("pcaxyz.plotly", width=500, height=500))),
                                         tags$br(),
                                         tags$hr(),
                                         tags$br(),
                                         fluidRow( column(12, h3('Static figure:'))),
                                         fluidRow( column(1, h4('Export')),  column(11)),
        ...
        output$filter.type <- renderUI({
            ...
            if(input$which.test != 'none')
                list(selectInput('filter.type', 'Filter based on:', c('nom.p', 'adj.p', 'top.n', 'none'), selected=ifelse(global.param$which.test == 'none', 'none', 'adj.p')))
            else
        ...
        output$list.groups <- renderUI({
            ...
            if( !global.param$grp.done ) return()
            list(
                radioButtons('log.transform', 'log-scale', choices=c('none', 'log10', 'log2'), selected='none'),
        ...
        observeEvent(input$run.test, {
            ...
            if(log.trans != 'none'){
                ...
                ## putting a dataframe around turns out to be ESSENTIAL!! DON'T USE CBIND HERE!
            ...
            if(test == 'Two-sample mod T'){
                ...
                withProgress(message='Two-sample test', value=0, {
                    ...
                    for(g in unique(groups.comp)){
                        ...
                        groups.tmp <- groups[ c(grep(unlist( strsplit(g, '\\.vs\\.'))[1] , groups), grep(unlist( strsplit(g, '\\.vs\\.'))[2] , groups)) ]
 ...
 ## filtered
        ...
        plotCorrMat <- function(filename=NA, lower=c('pearson', 'spearman', 'kendall', 'pcor'), upper=c('pearson', 'spearman', 'kendall', 'pcor'), trans=F, display_numbers=T){
            ...
            if(trans)
                pheatmap(cm, fontsize_row=10, fontsize_col=10,
                     cluster_rows=Rowv, cluster_cols=Colv, border_col=NA, col=color.hm, filename=filename, labels_col=chopString(colnames(cm), STRLENGTH), labels_row=chopString(rownames(cm), STRLENGTH), main='', display_numbers=display_numbers, fontsize_number=100/ncol(cm)+10)
            else
                pheatmap(cm, fontsize_row=10, fontsize_col=10,
        ...
        output$HM <- renderPlot({
            ...
            if(input$hm.max){
                 withProgress({
                     setProgress(message = 'Processing...', detail= 'Generating Heatmap')
                     plotHM(res=res, grp=global.param$grp, grp.col=global.param$grp.colors, grp.col.legend=global.param$grp.colors.legend,  hm.clust=input$hm.clust, hm.title=hm.title, hm.scale=input$hm.scale, cellwidth=40, fontsize_row=input$cexRow, fontsize_col=input$cexCol, max.val=input$hm.max.val, style=global.param$which.test)
                 })
            } else {
                 withProgress({
                     setProgress(message = 'Processing...', detail= 'Generating Heatmap')
                     plotHM(res=res, grp=global.param$grp, grp.col=global.param$grp.colors, grp.col.legend=global.param$grp.colors.legend,  hm.clust=input$hm.clust, hm.title=hm.title, hm.scale=input$hm.scale, cellwidth=40, fontsize_row=input$cexRow, fontsize_col=input$cexCol, style=global.param$which.test)
                 })
            }
        },
