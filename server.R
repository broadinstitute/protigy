...
shinyServer(
    ...
    function(input, output, session) {
        ...
        observeEvent(input$export.results, {
            ...
            ##                   volcanos
            ############################################################
            if(input$export.volc & global.param$which.test != 'mod F'){
	    ...
	    ##fn.all.abs <- grep('pdf$|xlsx$|RData$|txt$', dir(global.param$session.dir, full.names=T), value=T)
            fn.all <- grep('pdf$|xlsx$|txt$',  dir(global.param$session.dir) , value=T)
	    fn.all.abs <- grep('pdf$|xlsx$|txt$', dir(global.param$session.dir, full.names=T), value=T)
