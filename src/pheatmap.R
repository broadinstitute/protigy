#############################################################################
## forked from GitHub repos 'raivokolde/pheatmap'
##
#############################################################################
lo = function(rown, coln, nrow, ncol, cellheight = NA, cellwidth = NA, treeheight_col, treeheight_row, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row, fontsize_col, gaps_row, gaps_col, gapsize_row, gapsize_col, ...){
    ...
    if(is.na(cellwidth)){
        ...
        mat_width = unit(cellwidth * ncol, "bigpts") + length(gaps_col) * unit(gapsize_col, "bigpts")
    }
    ...
    if(is.na(cellheight)){
        ...
        mat_height = unit(cellheight * nrow, "bigpts") + length(gaps_row) * unit(gapsize_row, "bigpts")
    }
    ...
    cw = convertWidth(mat_width - (length(gaps_col) * unit(gapsize_col, "bigpts")), "bigpts", valueOnly = T) / ncol
    ch = convertHeight(mat_height - (length(gaps_row) * unit(gapsize_row, "bigpts")), "bigpts", valueOnly = T) / nrow
...
############################################################################
find_coordinates = function(n, gaps, m = 1:n, gapsize){
    if(length(gaps) == 0){
    ...
    size = (1 / n) * (unit(1, "npc") - length(gaps) * unit(gapsize, "bigpts"))

    gaps2 = apply(sapply(gaps, function(gap, x){x > gap}, m), 1, sum)
    coord = m * size + (gaps2 * unit(gapsize, "bigpts"))

    return(list(coord = coord, size = size))
}
##############################################################################
draw_dendrogram = function(hc, gaps, horizontal = T, gapsize){
    ...
    x = find_coordinates(n, gaps, x * n, gapsize=gapsize)$coord
    y = unit(y, "npc")
...
##############################################################################################
draw_matrix = function(matrix, border_color, gaps_rows, gaps_cols, fmat, fontsize_number, number_color, gapsize_row, gapsize_col){
    n = nrow(matrix)
    m = ncol(matrix)

    coord_x = find_coordinates(m, gaps_cols, gapsize=gapsize_col)
    coord_y = find_coordinates(n, gaps_rows, gapsize=gapsize_row)
...
##############################################################################
draw_colnames = function(coln, gaps, gapsize_col, ...){
    coord = find_coordinates(length(coln), gaps, gapsize=gapsize_col)
...
################################################################################
draw_rownames = function(rown, gaps, gapsize_row, ...){
    coord = find_coordinates(length(rown), gaps, gapsize=gapsize_row)
...
################################################################################
draw_annotations = function(converted_annotations, border_color, gaps, fontsize, horizontal, gapsize_col, gapsize_row){
    n = ncol(converted_annotations)
    m = nrow(converted_annotations)

    coord_x = find_coordinates(m, gaps, gapsize=gapsize_col)
...
heatmap_motor = function(matrix, border_color, cellwidth, cellheight, tree_col, tree_row, treeheight_col, treeheight_row, filename, width, height, breaks, color, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row, fontsize_col, fmat, fontsize_number, number_color, gaps_col, gaps_row, labels_row, labels_col, gapsize_row, gapsize_col, ...){
    ...
    if(!is.na2(tree_col) & treeheight_col != 0){
        elem = draw_dendrogram(tree_col, gaps_col, horizontal = T, gapsize_col)
        res = gtable_add_grob(res, elem, t = 2, l = 3, name = "col_tree")
    }

    # Draw tree for the rows
    if(!is.na2(tree_row) & treeheight_row != 0){
        elem = draw_dendrogram(tree_row, gaps_row, horizontal = F, gapsize_row)
        res = gtable_add_grob(res, elem, t = 4, l = 1, name = "row_tree")
    }

    # Draw matrix
    elem = draw_matrix(matrix, border_color, gaps_row, gaps_col, fmat, fontsize_number, number_color, gapsize_row=gapsize_row, gapsize_col=gapsize_col)
    res = gtable_add_grob(res, elem, t = 4, l = 3, clip = "off", name = "matrix")

    # Draw colnames
    if(length(labels_col) != 0){
        pars = list(labels_col, gaps = gaps_col, fontsize = fontsize_col,  gapsize_col=gapsize_col, ...)
    ...
    if(length(labels_row) != 0){
        pars = list(labels_row, gaps = gaps_row, fontsize = fontsize_row, gapsize_row=gapsize_row, ...)
    ...
    if(!is.na2(annotation_col)){
        ...
        elem = draw_annotations(converted_annotation, border_color, gaps_col, fontsize, horizontal = T, gapsize_col=gapsize_col, gapsize_row=gapsize_row)
    ...
    if(!is.na2(annotation_row)){
        ...
        elem = draw_annotations(converted_annotation, border_color, gaps_row, fontsize, horizontal = F, gapsize_row=gapsize_row, gapsize_col=gapsize_col)
